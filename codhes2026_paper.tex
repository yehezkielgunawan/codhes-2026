...
Eight of ten platform pairs favor machine documentation in Table~\ref{tab:platform-lri}. Vercel is the exception (42.00 machine versus 54.00 human), and LangChain is tied at 50.00. FastAPI and Cursor reach the scale ceiling of 100.00, which may reflect favorable ratings but also warrants caution about score compression. These exceptions show that format labels alone do not determine evaluator ratings.

The positive deltas range from 13.00 for React and FastAPI to 26.25 for GitHub. This spread, together with the negative Vercel delta, indicates substantial platform-level heterogeneity around the positive mean. Table~\ref{tab:platform-lri} reports all platform values to avoid inferring domain effects from individual cases.

\subsection{Correlation Analysis}

No statistically significant correlation was detected between traditional metrics and LLM ratings (all $p>0.05$). FRE and LLM-Friendliness showed $r=0.357$ ($p=0.312$); TWR and LLM-Friendliness showed the largest observed coefficient, $r=0.564$ ($p=0.090$), which fell to $r=0.312$ ($p=0.412$) after removing the React and LangChain TWR outliers. Figure~\ref{fig:fre-clarity} illustrates that low-FRE machine files can still receive high Clarity ratings, but the small sample does not establish independence between constructs.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{readability-python-demo/results/figures/figure3_fre_vs_clarity.png}
    \caption{Flesch Reading Ease versus LLM Clarity for human (pink) and machine (gold) documentation. Low-FRE machine files can still receive high Clarity ratings, illustrating divergence between the measures.}
