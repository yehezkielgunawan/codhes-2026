...
% Abstract
\begin{abstract}
This study presents a technolinguistic analysis comparing human-readable HTML documentation and machine-optimized llm.txt files across ten software engineering documentation platforms. Using established readability metrics (Flesch Reading Ease, Flesch-Kincaid Grade Level, Lexical Density) alongside a novel LLM-as-a-Judge evaluation methodology, we examine the readability paradox inherent in machine-optimized documentation. This analysis reveals that while machine-optimized documentation scores significantly lower on traditional human readability metrics (FRE mean difference: -91.58 points), it receives substantially higher ratings from LLM evaluators (LLM Readability Index mean difference: +14.69 points, p=0.005). Large effect sizes (Cohen’s d ranging from -1.002 to -1.371) across all LLM evaluation dimensions suggest that machine-optimized documentation is perceived as clearer, more concise, and more LLM-friendly. Crucially, there’s no significant correlation between traditional readability metrics and LLM evaluation scores, indicating these constructs measure fundamentally different aspects of documentation quality. These findings provide empirical evidence of the readability-tokenization paradox and suggest that traditional readability metrics are insufficient to evaluate modern documentation standards designed for both human and machine consumption.
\end{abstract}
...
The rapid emergence of large language models (LLMs) and artificial intelligence has drastically changed how software engineering documentation is used, analysed, and assessed. Conventional documentation adheres to established readability standards and formatting rules that are optimised for human cognition because it is primarily intended for human readers. But with the growth of machine-optimized documentation standards and LLM-as-a-Judge paradigms, a crucial issue emerges: Does machine-optimized documentation compromise human readability for computer speed, and if so, to what degree?

An important turning point in the development of documentation formats was Jeremy Howard's September 2024 announcement of llm.txt standards. Specifically created for LLM consumption at inference time, this standard suggests a machine-optimized format that offers succinct, expert-level information in a single accessible spot. The fundamental idea is that LLMs gain from better organised, token-efficient documentation that preserves technical correctness while removing language duplication. The readability conundrum, or the possible conflict between machine-optimized and human-readable documentation, is crucial to this optimisation, though.

Readability has been a cornerstone of technical communication since Rudolf Flesch produced the Flesch Reading Ease formula in 1948, and J. Peter Kincaid created the Flesch-Kincaid Grade Level in 1975. These measures have been extensively accepted in technical documentation, with industry standards suggesting Flesch Reading Ease scores ranging from 60 to 70 for general technical material and 8 to 10 for developer audiences. However, these measures were designed for schoolbooks and general writing, not for the specialised context of machine-optimized documentation.

Text tokenisation, notably Byte Pair Encoding (BPE), developed by Sennrich, Haddow, and Birch in 2016, has become an important concern in documentation design. BPE generates a variable-length encoding in which common words use fewer tokens than unusual or complex terms. This has a direct impact on LLM processing efficiency, as token count correlates with API charges, latency, and context window utilisation. The "Lost in the Middle" phenomenon, observed by Liu et al. (2023), reveals that LLM performance declines when vital information is hidden in extensive contexts, emphasising the importance of compact, front-loaded documentation forms.

Tokenisation of text, notably using Byte Pair Encoding (BPE), developed by Sennrich, Haddow, and Birch in 2016, has become an important concern in documentation design. BPE generates a variable-length encoding, with common words requiring fewer tokens than unusual or complicated terms. This has a direct influence on the efficiency of LLM processing, since token count correlates with API charges, latency, and context window utilisation. The ''Lost in the Middle'' phenomenon, as demonstrated by Liu et al. (2023), reveals that LLM performance declines when vital information is hidden in extensive contexts, emphasising the need of compact, front-loaded documentation forms.

This work fills a crucial research gap by performing a quantitative computational analysis of human-readable HTML documentation against machine-optimized llm.txt files across ten software engineering documentation platforms. To provide a comprehensive assessment of the readability-tokenization paradox, we use a paired comparison design that controls for content differences by comparing versions of the same documentation. We also use traditional readability metrics and a novel LLM-as-a-Judge evaluation methodology.

The methodology used in the study is intended to guarantee methodological rigour and repeatability. We created a unique Python CLI tool called readability-python-demo that automates every step of the data collecting process, from site crawling and URL identification to metrics computation and LLM assessment. The contemporary Python package management system uv (Astral), which offers quick dependency resolution and clean environment isolation, is used in the tool's construction. Fifty pytest tests spanning all important components are included in the source, guaranteeing implementation accuracy and making reproducibility easier for other researchers.

Frontend frameworks (React), backend frameworks (FastAPI, Hono), developer platforms (GitHub, Vercel), infrastructure services (Cloudflare), payment systems (Stripe), AI tools (Cursor, LangChain), and backend-as-a-service platforms (Supabase) are among the platforms chosen for analysis. These platforms were chosen in particular because they cater to audiences in software engineering and offer technical documentation on frameworks, deployment platforms, APIs, and developer tools. By ensuring that the results are not restricted to a particular software engineering sub-domain or technology stack, this variety improves the results' generalisability throughout the larger software engineering documentation ecosystem.

Beyond scholarly curiosity, this study is significant. The quality of machine-optimized documentation significantly affects the quality of AI-generated responses as LLM-based AI assistants increasingly mediate developer access to software engineering documentation. Inadequate machine documentation can result in LLM hallucinations, improper use of APIs, or insufficient information retrieval, all of which have an impact on program quality and developer productivity. On the other hand, when working with AI-powered tools, well-optimized machine documentation can boost LLM performance, lower API costs, and improve the developer experience in general.

Given the swift application of AI helpers in software engineering workflows, the study is very pertinent. Technical documentation is now regularly consumed by GitHub Copilot, ChatGPT, Claude, and other AI technologies to offer code recommendations, respond to enquiries, and resolve problems. The calibre of the documentation that the AI consumes has an impact on the calibre of these AI-generated responses. The AI may find it difficult to extract essential information from poorly optimised content, which could result in inaccurate or subpar answers. This paper presents a framework for evaluating the quality of machine-optimized documentation as well as empirical support for its significance.

Additionally, by analysing how documentation formats change to accommodate new customer kinds, the study advances the fields of software engineering and human-computer interaction. Documentation is changing once more to serve AI users, just as it changed from print manuals to HTML web pages to serve online readers. The future of technical communication and the place of human writers in an era of AI-mediated consumption are two significant issues raised by this trend. The study's conclusions lay the groundwork for additional investigation into these issues and offer helpful advice for documentation teams going through this change.

The development of documentation formats reflects more general changes in information consumption brought about by technology. From the 1960s until the 1990s, print manuals predominated. They had extensive indexes and a table of topics and were intended for linear reading. Hypertext navigation, search capabilities, and multimedia integration were made possible by the switch to HTML-based documentation in the late 1990s, which drastically altered how users interacted with technical information. New assessment techniques were needed for each shift: web documentation introduced measures like click-through rates, time-on-page, and search engine optimisation scores, while print manuals were evaluated using user comprehension exams and job completion durations. Through the development of the LLM Readability Index and the methodical comparison of human and machine assessment metrics, this work introduces yet another paradigm shift in evaluation that is necessary for the present shift to machine-optimized documentation.

This change in documentation has equally important economic consequences. Enterprise documentation teams, which usually consist of dozens of technical writers, editors, and information architects, devote significant resources to the creation and upkeep of software engineering documentation. New cost issues are brought about by the introduction of machine-optimized documentation standards: can a single format satisfy both audiences, or should organisations maintain multiple documentation formats (human-readable and machine-optimized)? The study's conclusions offer information to guide these tactical choices, proving that the readability-tokenization conundrum is a real issue with quantifiable effects on the effectiveness and quality of documentation.
...
\textbf{RQ1:} Do machine-optimized documentation files (llm.txt) have significantly different traditional readability scores than their human-readable HTML counterparts?

\textbf{RQ2:} Do the LLM-as-a-Judge evaluation scores favour machine-optimized documentation over human-readable HTML documentation?

\textbf{RQ3:} Are existing readability measures (Flesch Reading Ease, Flesch-Kincaid Grade Level) enough for assessing documentation intended for both human and machine consumption, or do they fail to reflect LLM-perceived quality dimensions?
...
This study advances the nascent area of technolinguistics by offering empirical evidence for the readability-tokenization conundrum. The findings have ramifications for technical writers, documentation platform developers, and LLM researchers, as they provide insights into the trade-offs between human readability and machine optimisation. In addition, the unique LLM-as-a-Judge approach and LLM Readability Index (LRI) quantification are beneficial instruments for future documentation evaluation study.
...
In September 2024, Jeremy Howard introduced the llm.txt standard, which marks a paradigm shift in documentation design \cite{Howard2024}. The standard demands a unique Markdown structure that provides brief, expert-level information in a single, easily accessible area, specifically designed for LLM consumption during inference rather than training. According to the official proposal, "Large language models increasingly rely on website information, but face a critical limitation: context windows are too small to handle most websites in their entirety." Converting sophisticated HTML pages containing navigation, advertisements, and JavaScript into LLM-friendly plain text is challenging and imprecise."

Major IT companies like Vercel, Cloudflare, Postman, Cursor, Anthropic, and Hugging Face \cite{AnswerDotAI2024} have adopted llm.txt, as evidenced by the over 2,088 repositories on GitHub that include llm.txt files. The ecosystem has grown beyond the core standard to include several auto-generation tools for VitePress, Docusaurus, and other documentation systems \cite{llmstxtSite,llmstxtDirectory}, llms-full.txt (expanded versions), and llms-ctx.txt (XML-structured context).

Our research quantifies the tension among human and machine optimisation, which is expressly framed by this norm. Websites are useful for both human readers and LLMs, although the latter gain from having access to more succinct, expert-level information in one convenient spot. The explicit standardisation for machine consumption efficiency is demonstrated by the semantic meaning of the ``Optional'' section in the llm.txt format, which permits content to be omitted for shorter context.

A H1 title, an H2 summary with succinct professional information, and an optional H2 section with more details that can be omitted for shorter contexts make up the standard llm.txt format. Traditional HTML documentation, that mixes navigation, ads, and interactive features with the real material, stands in stark contrast to this structured approach. The LLM ecosystem has matured with the standardisation of machine-optimized documentation, which recognises that LLMs are active participants who need purpose-built information formats rather than just passive consumers of web content.

The llm.txt adoption trend points to a quick change in business practices. Major internet businesses like Vercel, Cloudflare, and Stripe implemented llm.txt files on their documentation webpages within six months of the standard's proposal. This quick uptake shows that, despite the possible loss of human readability, the technical documentation community values machine-optimized forms. Additionally, the ecosystem has produced automated generation tools that reduce the amount of manual labour needed for implementation by converting current documentation into llm.txt format.

\subsection{Traditional Readability Metrics}

The Flesch Reading Ease (FRE) formula, created by Rudolf Flesch in 1948 \cite{Flesch1948}, and the Flesch-Kincaid Grade Level (FKGL), created by J. Peter Kincaid et al. in 1975 \cite{Kincaid1975}, are the cornerstones of contemporary readability evaluation. Sentence length and syllable count per word are used by the FRE formula to determine readability, and FKGL converts these metrics into grade level equivalents in the United States.

These criteria have earned academic credibility. FKGL became a US military standard in 1978 after Flesch's initial research was published in the Journal of Applied Psychology. The U.S. Department of Defence, Microsoft Word, Grammarly, and IBM Lotus Symphony have all embraced these measures. However, Redish (2000) \cite{Redish2000} and McClure (1987) \cite{McClure1987} point out that these formulas have serious drawbacks when used for technical documentation: they were created for school books, not technical content; they ignore reader differences and the effects of content, layout, and retrieval aids; and they assume English text with limited applicability to non-English contexts.

Industry standards for technical documentation suggest grade levels of 8–10 for general developer audiences and FRE scores of 60–70 for general documentation \cite{Kincaid1988}. However, because the subject matter is difficult and the formulae have trouble with polysyllabic technical phrases, technical material inevitably receives lower results. When assessing machine-optimized documentation, which may purposefully utilise dense, succinct language that conventional metrics interpret as less legible, this restriction is very important.

Alternative methods have been developed in response to the shortcomings of conventional readability measurements. While the Automated Readability Index (ARI), Coleman-Liau Index, and Gunning Fog Index provide varying weights for vocabulary difficulty and sentence complexity, they all fundamentally rely on surface-level linguistic traits rather than semantic or pragmatic qualities. Cohesion and coherence measures are incorporated into more contemporary methods like Coh-Metrix, although these instruments are computationally demanding and require specialised software.

It is especially dubious if typical measurements can be used to machine-optimized documentation. Text that has been machine-optimized frequently employs succinct, urgent phrase forms with technical jargon that LLMs can process quickly but humans could find challenging. For instance, the statement "Configure the API key in environment variables" has a low Flesch Reading Ease score because of its complicated noun phrase and technical language, yet LLMs are adept at handling this type of straightforward, practical instruction. The readability-tokenization conundrum examined in this paper is theoretically based on this discrepancy between what conventional metrics measure and what machine users need.

Alternative methods for evaluating readability that might more accurately reflect machine-relevant aspects have been provided by recent developments in computational linguistics. Although the Coleman-Liau Index and the Simple Measure of Gobbledygook (SMOG) provide alternative linguistic complexity weightings, they are still primarily based on human reading comprehension rather than machine processing efficiency. Graesser and McNamara's Coh-Metrix provides a more detailed evaluation of text quality that may partially overlap with machine readability by measuring cohesion and coherence using discourse markers and semantic overlap. Nevertheless, Coh-Metrix is computationally demanding and necessitates certain linguistic processing pipelines that are difficult to implement for extensive documentation analysis.

Neural network-based readability models are a promising approach that connects machine evaluation with conventional measures. These approaches estimate readability scores using pre-trained language models, which may capture contextual and semantic elements missed by formula-based measures. These models, however, have the same human-centric biases that restrict conventional metrics as they were trained on human readability assessments. The basic problem still exists: readability was established and operationalised for human readers, therefore expanding it to machine consumers necessitates either redefining the concept or creating completely new assessment frameworks. The LLM Readability Index, a complementary metric that reflects machine-perceived quality without claiming to assess the same construct as traditional readability, is proposed in this work to solve this difficulty.

\subsection{Tokenization and Computational Efficiency}

The standard tokenisation technique for contemporary LLMs is Byte Pair Encoding (BPE), which was first presented to NLP by Sennrich, Haddow, and Birch in 2016 \cite{Sennrich2016}. BPE is lossless, reversible, and compresses text, usually producing tokens that are about 4 bytes in size each \cite{OpenAI2023}. Variable-length encoding is a crucial feature: ordinary words only need one token, whereas rare or complicated words need several. Recent research has examined how tokenisation choices impact model behaviour \cite{Ankner2024}, and the selection of subword merge operations has a substantial impact on downstream performance \cite{Ding2019}.

Documentation design is significantly impacted by the token economy. Token count is directly correlated with API cost and delay, according to research, and well-structured, token-efficient prompts outperform verbose prompts by 10–15\% while costing 5–10 times less. The necessity for succinct, front-loaded documentation formats like llm.txt \cite{Liu2023} is directly motivated by the empirical evidence of the “Lost in the Middle” phenomena (Liu et al., 2023), which shows that LLMs struggle to use information in the middle of lengthy contexts.

Published at EMNLP 2023, LLMLingua (Jiang et al., 2023) demonstrates that well-structured compression may retain semantic integrity while dramatically lowering token count \cite{Jiang2023}, achieving up to 20x prompt compression with minimal performance loss. Nevertheless, this study concentrates on compression methods rather than documentation standards, which leaves a vacuum in our knowledge of how choices in documentation format impact machine efficiency and human readability.

\subsection{LLM-as-a-Judge Evaluation Methodology}

One methodological advancement in documentation assessment is the employment of LLMs as assessors. Recent research has shown that LLMs can offer dependable, consistent assessments of text quality in a variety of contexts. This approach provides a machine-centric view of documentation quality that supplements conventional human-focused metrics by utilising the intrinsic capacity of LLMs to evaluate clarity, completeness, conciseness, and technical correctness.
...
The LLM evaluation employs a five-dimension Likert scale assessment via the OpenRouter API \cite{OpenRouter2024}, using the meta-llama/llama-3.2-3b-instruct model. The evaluation dimensions are:
